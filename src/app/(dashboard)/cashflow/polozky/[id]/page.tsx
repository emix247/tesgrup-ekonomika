...
export default function EditPolozka() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/cashflow/items/${id}`).then(r=>r.json()),
      fetch('/api/cashflow/projects').then(r=>r.json()),
    ]).then(([item, projs]) => {
      setForm({ type: item.type, name: item.name, amount: String(item.amount), paymentDate: item.paymentDate?.split('T')[0] || '', certainty: item.certainty, category: item.category, projectId: item.projectId ? String(item.projectId) : '', counterparty: item.counterparty || '', note: item.note || '' });
      setProjects(projs);
      setLoading(false);
    });
  }, [id]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`/api/cashflow/items/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
