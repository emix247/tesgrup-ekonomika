...
export async function GET() {
  return NextResponse.json(await getOverheadAllocations());
}
...
export async function POST(request: NextRequest) {
  ...
  const item = await upsertOverheadAllocation(parsed.data);
...
export async function PUT(request: NextRequest) {
  ...
  const updated = await upsertOverheadAllocation({ id, ...data });
...
export async function DELETE(request: NextRequest) {
  ...
  if (!id) return NextResponse.json({ error: 'ID je povinné' }, { status: 400 });
  await deleteOverheadAllocation(id);
